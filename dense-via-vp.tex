...
This is a variant of the Variable Projection algorithm of \citet{1973SJNA...10..413G}, but I will derive it directly from the jointcal astrometry objective function \prettyref{eq:astrometry-chi2} to ensure the slight differences to the sparse solution are apparent.
...
As discussed below in \prettyref{subsec:vp_local_linearity}, $A_i$ should be approximately constant across iterations, so we should actually be able to compute the per-star QR factorizations once up front.

\subsection{Implementation Concerns}

\subsubsection{Minimizing Refactoring}
...
This may rule out or weaken some of the potential optimizations described in \prettyref{sec:vp_further_optimization}, but as those mostly correspond to operations that are currently negligible in the compute budget, that's a reasonable starting point.
